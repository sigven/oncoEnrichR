...
* Each target in the query set is annotated with:
   ...
   * Classification with respect to roles as tumor suppressors, proto-oncogenes, and potential cancer driver genes
        * Cancer driver genes need support from at least two of the following sources:
          ...
          * [CancerMine](https://www.nature.com/articles/s41592-019-0422-y) literature mining resource (considering only entries with >= 15 citations)
          * Genes found in a curated list of [false positive driver predictions](https://pubmed.ncbi.nlm.nih.gov/29625053/) are ignored as potential drivers
        * Tumor suppressor genes and proto-oncogenes require support from at least two of the following sources:
           * [CancerMine](https://www.nature.com/articles/s41592-019-0422-y) literature mining resource (considering only entries with >= 15 citations), 
           * [Network of Cancer Genes](http://ncg.kcl.ac.uk/)
           * [Cancer Gene Census](https://cancer.sanger.ac.uk/census)
...
target_assocs <- onc_enrich_report[['data']][['disease']][['target']] |>
  dplyr::mutate(targetset_cancer_rank = round(.data$targetset_cancer_rank, digits = 2)) |>
  dplyr::mutate(global_cancer_rank = round(.data$global_cancer_rank, digits = 2))
