...
* The [Cell Model Passports](https://cellmodelpassports.sanger.ac.uk/) resource represent a hub for preclinical cancer models, primarily including cell lines, with rich molecular and phenotypic annotations, including gene fitness data from the cancer [Dependency Map](https://depmap.org).
* Here, we are showing, across the main human tissue types:
   ...
   * The underlying cell viability data comes from the integration of CRISPR dependency scores in n = 921 cell lines (Sanger/Broad), leveraging [DepMap (2021_Q2)](https://score.depmap.sanger.ac.uk/downloads), as published in [Pacini et al., Cancer Cell, 2024](https://pubmed.ncbi.nlm.nih.gov/38215750/)
...
* A maximum of **n = 200** genes with the highest number of tissue-specific cell lines with significant loss-of-fitness effect are shown in the plot below (the complete set of genes, including the loss-of-fitness scores per cell line, can be found in the Excel output of oncoEnrichR)
...
top_fitness_hits <- targets_wide |>
  dplyr::mutate(symbol = forcats::fct_reorder(
    symbol, n_models_dependent_total)) |>
...
fitness_lof_oe_plot <- targets_wide |>
  dplyr::mutate(
    symbol = forcats::fct_reorder(symbol, n_models_dependent_total)) |>
  dplyr::inner_join(top_fitness_hits, by = "symbol") |>
  ggplot2::ggplot(
    ggplot2::aes(x = symbol, y = n_models_dependent_tissue, fill = tissue)) +
  ggplot2::geom_bar(stat = "identity") +
  ggplot2::coord_flip() +
  ggplot2::ylab("Number of cell lines with significant loss-of-fitness") +
  ggplot2::xlab("") +
  ggplot2::scale_fill_manual(values = oncoEnrichR::color_palette$tissue) +
  ggplot2::theme(
...
* Here we list target genes defined as _common essential_ (i.e. genes that exert a loss-of-fitness
effect across many cell lines and are essential for the survival of most cell types, regardless of their cancer type), as defined in [Pacini et al., Cancer Cell, 2024](https://pubmed.ncbi.nlm.nih.gov/38215750/)

* **`r round((onc_enrich_report[['data']][['fitness']][['fitness_scores']][['n_essential']] / NROW(onc_enrich_report$data$query$target)) * 100, digits = 1)`%** of genes in the query set are annotated as common essential genes in DepMap/Project Score

```{r fitness_common_essential}
#| echo: false
#| output: asis
#| eval: !expr onc_enrich_report[['data']][['fitness']][['fitness_scores']][['n_essential']] > 0

htmltools::br()

fitness_common_essential_df <- 
  onc_enrich_report[['data']][['fitness']][['fitness_scores']][['common_essential']] |>
  dplyr::rename(symbol = symbol_link_ps) |>
  head(500)
...
* Note that we here omit entries for _common essential genes_ (i.e. genes that are essential for the survival of most cell types, regardless of their cancer type), as defined in [Pacini et al., Cancer Cell, 2024](https://pubmed.ncbi.nlm.nih.gov/38215750/)
...
fitness_lof_oe_df <- 
  onc_enrich_report[['data']][['fitness']][['fitness_scores']][['targets']] |>
  dplyr::filter(.data$essential_gene == F) |>
  dplyr::group_by(.data$symbol,
                  .data$n_models_dependent_tissue,
                  .data$symbol_link_ps,
                  .data$n_models_dependent_total,
  ...
  dplyr::summarise(ps_link = paste(.data$model_link_ps, collapse = ", "),
                   .groups = "drop") |>
  dplyr::ungroup() |>
  dplyr::select(c("symbol_link_ps","tissue", "ps_link")) |>
  dplyr::rename(cell_lines = ps_link, 
                symbol = symbol_link_ps) |>
...
* Promising candidate therapeutic targets are indicated through __[target priority scores](https://score.depmap.sanger.ac.uk/documentation#scores)__. Target priority scores are based on integration of CRISPR knockout gene fitness effects with genomic biomarker and patient data ([Pacini et al., Cancer Cell, 2024](https://pubmed.ncbi.nlm.nih.gov/38215750/)). All genes are assigned a target priority score between 0 – 100 from lowest to highest priority. In the heatmap shown below, genes in the query set are ranked according to their respective priority scores across all cancers (i.e. _Pancancer_), limited to the top 100 candidates.
