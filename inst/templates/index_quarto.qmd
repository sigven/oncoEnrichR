...
include-in-header:
  - text: |
      ...
      /* Force a single, consistent zebra-stripe shade on all tables,
         regardless of which DT style variant a given module happens to
         load (different variants use different striping classes -
         table-striped vs. stripe/display - with different opacities,
         which otherwise looks inconsistent across sections). */
      table.dataTable > tbody > tr.odd > * {
        box-shadow: inset 0 0 0 9999px rgba(0, 0, 0, 0.06) !important;
      }
      </style>
      <script>
      /* Disable zenscroll's global click hijacking (bundled transitively via
         an htmlwidget dependency). It intercepts clicks on ANY <a href="#">
         anywhere on the page - including DataTables' pagination buttons,
         which use a bare href="#" - and scrolls the page to the very top.
         Must be set before zenscroll's own script executes, hence placed in
         include-in-header rather than an after-body script. */
