...
* Using data from the [Human Protein Atlas (HPA) - Cell/Tissue Atlas](https://www.proteinatlas.org/humanproteome/tissue), we are here interrogating classification of all protein coding genes in the query set with respect to elevated expression in normal/healthy tissues and cell types
  * Gene expression per tissue is based on data from [Genotype-Tissue Expression (GTex) Project](https://gtexportal.org/home/), containing data from [36 distinct tissue types](https://www.proteinatlas.org/about/assays+annotation#gtex_rna). Gene expression measure unit: [Transcripts pr million (TPM)](https://www.proteinatlas.org/about/assays+annotation#normalization_rna)
...
tissue_specs_per_gene <- 
  crosstalk::SharedData$new(onc_enrich_report[['data']][['cell_tissue']][['tissue_enrichment']][['per_gene']])
...
* Genes have been classified, based on mean expression (across samples) per cell type, into distinct _specificity categories_ (algorithm developed within HPA):
  ...
  * **Cell type enhanced**: Genes with a mean expression level greater than or equal to 1 (_NX >= 1_) that also have at least four-fold higher expression levels in a particular cell type compared to the average levels in all other cell types, and that are not considered *Cell type enriched* or *Group enriched*.
