...
* Based on data recently calculated by [Smith et al., *bioRxiv*, 2021](https://www.biorxiv.org/content/10.1101/2021.06.01.446243v1), we here show the relative prognostic implications of genes in the query set, for different genetic features ( here limited to **expression**, **mutation** and **CNA**).
  * The data provided by [Smith et al.](http://survival.cshl.edu) was harvested through analysis of TCGA datasets, in which [Cox proportional hazards models](http://survival.cshl.edu/?faqs#How%20was%20this%20analysis%20conducted%3F) were generated linking the expression, copy number, or mutation status of every gene in the genome with patient outcome in different cancer types profiled by the TCGA.
  * Kaplan-Meier curves were generated by dividing patients into two groups and comparing the survival times between each group. For RNA-Seq, the division is made based on the mean expression of the feature. For CNAs, the division is made based on the mean copy number of the feature. For mutations, the division is made between patients who have a mutation (protein-coding only) in a gene and patients who lack mutations in a gene.
  ...
  * Shown here are three heatmaps (CNA, expression, mutation) with prognostic **Z-scores** for members of the query set (limited to the top 100). Tumor type cohorts are designated with TCGA abbreviations (COAD (Colon Adenocarcinoma), BRCA (Breast Invasive Carcinoma) etc.) 
...
mutation_survival_fig <- plotly::plot_ly(
  ...
  plotly::colorbar(
    ...
    limits = c(plyr::round_any(min(mutation_zscore_matrix), -12, ceiling),
               plyr::round_any(max(mutation_zscore_matrix), 12, ceiling)))
...
expression_survival_fig <- plotly::plot_ly(
  ...
  plotly::colorbar(
    nticks = 10, 
    title = list(text = "Z-score",side = "bottom"),
   limits = c(plyr::round_any(min(expression_zscore_matrix), -12, ceiling),
               plyr::round_any(max(expression_zscore_matrix), 12, ceiling)))
...
cna_survival_fig <- plotly::plot_ly(
  ...
  plotly::colorbar(
    nticks = 10, 
    title = list(text = "Z-score",side = "bottom"),
    limits = c(plyr::round_any(min(cna_zscore_matrix), -12, ceiling),
               plyr::round_any(max(cna_zscore_matrix), 12, ceiling)))
