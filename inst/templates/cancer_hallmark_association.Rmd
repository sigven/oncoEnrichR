...
```{r filter_cancer_hallmarks, echo=F, results='asis'}

missing_hallmarks <- TRUE

if(nrow(onc_enrich_report[['data']][['cancer_hallmark']][['target']]) > 0){
  missing_hallmarks <- FALSE
}
...
```{r table_browse_hallmarks, echo=F, results = "asis", eval = !missing_hallmarks}
library(crosstalk)

target_hm_shared <- onc_enrich_report[['data']][['cancer_hallmark']][['target']]
...
```{r hallmarks_missing, echo=F, results = 'asis', eval = missing_hallmarks}
