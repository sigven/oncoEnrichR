...
```{r go_plot_missing_all, echo=F, results = 'asis', eval = !subontology_plots[['ALL']]}

cat('<br><br>\n <ul><li>  <i> <span style="font-size: 105%; padding: 3px; background-color:#989898; color:white">&nbsp;&nbsp;Enrichment plot omitted: No GO terms were enriched in the query set.&nbsp;&nbsp; </span></i></li></ul>',sep='\n')
cat('\n')
cat('<br><br>')
...
plot_data <- onc_enrich_report[['data']][['enrichment']][['go']] %>%
  ...
  dplyr::filter(db == 'GO_MF') %>%
  dplyr::arrange(qvalue) %>%
  utils::head(onc_enrich_report[['config']][['enrichment']][['num_terms_enrichment_plot']]) %>%
  dplyr::arrange(desc(qvalue)) %>%
  dplyr::rename(Subontology = db) %>%
  dplyr::mutate(
    Subontology = dplyr::case_when(
