...
```{r go_enrichment_barplot, echo = F, results = "asis", eval = show_enrichment_filters[['go']]}
...
plot_data <- onc_enrich_report[['data']][['enrichment']][['go']] %>%
  ...
  dplyr::mutate(
    Subontology = dplyr::case_when(
      Subontology == "GO_BP" ~ "Biologial Process",
...
p1 <- plotly::ggplotly(p, width = 800, height = height_plot) %>%
