...
annotate_subcellular_compartments <- function(qgenes,
  ...
  log4r_info("ComPPI: retrieval of subcellular compartments for target set")
  ...
  if(nrow(target_compartments_all) > 0){
    target_compartments_all <- as.data.frame(
      ...
      dplyr::left_join(
        ...
        by = "go_id") %>%
      dplyr::left_join(
        dplyr::select(genedb, .data$symbol,
                      .data$entrezgene,
                      .data$genename),
                       by = c("symbol")) %>%
        dplyr::mutate(
    ...
    anatogram_values <- as.data.frame(
      ...
      dplyr::ungroup() %>%
      dplyr::mutate(value = round((.data$n_comp / .data$n_genes) * 100, digits = 4)) %>%
