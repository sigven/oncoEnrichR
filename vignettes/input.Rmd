...
genes that can be used as input to the tool. If you run oncoEnrichR through the Galaxy interface, the upper limit is set at `n = 500`. Please also note that the tool requires `n = 2` input genes at the bare minimum, although some analysis modules will not be included/run with so few genes as input. 
